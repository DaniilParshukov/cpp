...
public:
    ...
    T get(int pos) {
        ...
        if (pos < lc) {
            ...
            return this->right_->get(pos - lc - 1);
        }
    }
...
public:
    ...
    private:
        ...
        int pos_;
    public:
        iterator(Node<H>* root, int pos): root_(root), pos_(pos) {}

        H operator*() {
            return root_->get(pos_);
        }
...
void testIterator1() {
    ...
    int sum = 0;
    Treap<int>::iterator it = treap.begin();
    Treap<int>::iterator end = treap.end();
